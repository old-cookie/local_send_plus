...
class DiscoveryService {
  ...
  bool _isDiscovering = false; // Flag indicating if discovery is currently active.
  final Set<String> _localIPs = {}; // Set of local IP addresses to filter self-discovery.
  final Map<String, Timer> _deviceExpiryTimers = {}; // Timers to track device timeouts.
  final Duration _deviceTimeout = const Duration(seconds: 15); // Duration after which a device is considered offline.
  final Duration _discoveryInterval = const Duration(seconds: 5); // Interval for sending discovery packets.
  final String _multicastAddress = '224.0.0.1'; // Multicast address for discovery.
  ...
  Future<void> startDiscovery() async {
    ...
    try {
      ...
      _socket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, _listenPort);
  ...
  Future<void> _sendDiscoveryPacket() async {
    ...
    final message = jsonEncode({'alias': alias, 'port': _listenPort, 'type': 'discovery_request'});
